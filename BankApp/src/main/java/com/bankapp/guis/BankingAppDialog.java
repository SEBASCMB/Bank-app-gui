...
public class BankingAppDialog extends JDialog implements ActionListener {
  ...
  public void addCurrentBalanceAndAmount(){
    ...
    String saldoFormateado = nf.format(user.getCurrentBalance());
    balanceLabel = new JLabel("Balance: COP " + saldoFormateado);
  ...
  private void resetFieldsAndUpdateCurrentBalance(){
    ...
    nf.setMaximumFractionDigits(0);
    String saldoFormateado = nf.format(user.getCurrentBalance());
    balanceLabel.setText("Balance: COP " + saldoFormateado);

    bankigAppGui.getCurrentBalancField().setText("COP " + saldoFormateado);
  ...
  public void actionPerformed(ActionEvent e){
    ...
    String rawInput = enterAmountField.getText().replace(".", "").replace(",", "");
    float amountVal = 0;
    try {
      amountVal = Float.parseFloat(rawInput);
    } catch (NumberFormatException ex) {
      JOptionPane.showMessageDialog(this, "Por favor ingresa un valor numérico válido en pesos colombianos.");
      return;
    }
